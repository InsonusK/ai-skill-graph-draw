...
| Поле | Тип | Обязательное | Описание |
|------|-----|--------------|----------|
| `engine` | string | Нет | Движок раскладки: `layered` (по умолчанию) или `igraph_sugiyama`. |
| `direction` | string | Нет | Направление рёбер: `LR` (по умолчанию), `RL`, `TB`, `BT`. |
